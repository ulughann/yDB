...
export class Collection {
    ...
    math(key, operator, value, goToNegative = false) {
        if (!/^-?\d+$/.test(`${value}`))
            throw Error(formatErrorMessage("Invlid Type Value", "Collection", "Method", "math"));
        if (!`${operator}`.match(/(\-|\+|\/|\*)/g))
            throw Error(formatErrorMessage("Invlid Type Operator", "Collection", "Method", "math"));
        if (typeof goToNegative != "boolean")
            throw Error(formatErrorMessage("Invlid Type goToNegative", "Collection", "Method", "math"));
        if (!this.has(key)) {
            this.set(key, value);
        }
        else {
